...
class BackupManager:
    ...
    def backup_redis_data(
        ...
        logger.debug(f"backup_redis_data called: backup_dir={backup_dir}, key_patterns={key_patterns}")
        try:
            if not backup_dir:
                backup_dir = os.path.join(config.settings.OUTPUT_DIR_PATH, "backups")
            os.makedirs(backup_dir, exist_ok=True)
    ...
    def restore_redis_backup(backup_path: str, overwrite: bool = False) -> Tuple[bool, str]:
        logger.debug(f"restore_redis_backup called: backup_path={backup_path}, overwrite={overwrite}")
        try:
    ...
    def apply_retention_policy(
        ...
        logger.debug(f"apply_retention_policy called: backup_dir={backup_dir}, days_to_keep={days_to_keep}, min_backups_to_keep={min_backups_to_keep}")
        try:
            if not backup_dir:
                backup_dir = os.path.join(config.settings.OUTPUT_DIR_PATH, "backups")
    ...
    def cleanup_old_files(
        ...
        logger.debug(f"cleanup_old_files called: output_dir={output_dir}, days_to_keep={days_to_keep}, file_patterns={file_patterns}")
        try:
            if not output_dir:
